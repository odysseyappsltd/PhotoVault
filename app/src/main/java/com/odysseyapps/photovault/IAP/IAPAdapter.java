...
public class IAPAdapter extends BaseAdapter {
    ...
    public IAPAdapter(Context context , BillingProcessor billingProcessor) {
        ...
        titles= IAPData.getSharedInstance().getTitles();
        subTitles = IAPData.getSharedInstance().getSubTitles();
        inflater =(LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        this.billingProcessor = billingProcessor ;
        demoPriceList = IAPData.getSharedInstance().getDemoPriceList();
    }

    @Override
    public int getCount() {
        return IAPData.getSharedInstance().getTitlesSize();
    }
